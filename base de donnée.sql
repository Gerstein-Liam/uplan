//////////////////////////////////////////////////////////////////////////////////////
select *from information_schema.table_constraints where constraint_schema = 'uplan_db'; 
SET FOREIGN_KEY_CHECKS=0;
TRUNCATE utilisateur;
TRUNCATE image;
SET FOREIGN_KEY_CHECKS=1;
///////////////////////////////////////////////////////////////////////////////////////
CREATE TABLE utilisateur (
		id_user MEDIUMINT NOT NULL AUTO_INCREMENT, 
		email varchar(100) NULL,
		username varchar(100) NULL,
		password varchar(100) NULL,
		quick_pres varchar(100) NULL,
	    id_photo_profil MEDIUMINT NULL,
		latitude FLOAT  NULL , longitude FLOAT  NULL,
		Primary Key (id_user)
		) ENGINE=InnoDB ; 


!puis mettre comme valeur par defaut NULL a latitude et 
 longitude depuis le tableau!
///////////////////////////////////////////////////////////////////////////////////////
CREATE TABLE IF NOT EXISTS `image` (
	   id_image MEDIUMINT NOT NULL AUTO_INCREMENT, 
	   fk_id_user MEDIUMINT NULL,
	  `image_name` varchar(32)     NULL,
	  `image` longblob      NULL,
	  `type` varchar(32)    NULL,
	  `size` varchar(32)    NULL,
  
	   PRIMARY KEY (`id_image`),
	   Foreign Key (fk_id_user) references utilisateur(id_user)
	   ) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=0 ;
   
ALTER TABLE image ADD CONSTRAINT c_fk_user FOREIGN KEY (fk_id_user) REFERENCES utilisateur(id_user) ON DELETE CASCADE ON UPDATE CASCADE;
///////////////////////////////////////////////////////////////////////////////////////
	
CREATE TABLE IF NOT EXISTS `publication` (
id_publication MEDIUMINT NOT NULL AUTO_INCREMENT, 
fk_id_user MEDIUMINT NULL,
username varchar(100) NOT NULL,	
text varchar(100) NOT NULL,
PRIMARY KEY (`id_publication`),
Foreign Key (fk_id_user) references utilisateur(id_user)
) ENGINE=InnoDB  ;

ALTER TABLE publication ADD CONSTRAINT c_fk_user FOREIGN KEY (fk_id_user) REFERENCES utilisateur(id_user) ON DELETE CASCADE ON UPDATE CASCADE;


